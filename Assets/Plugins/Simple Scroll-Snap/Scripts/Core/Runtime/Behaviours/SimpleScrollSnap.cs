...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        private void Setup()
        {
            ...
            for (int i = 0; i < NumberOfPanels; i++)
            {
                ...
                if (movementType == MovementType.Fixed && useAutomaticLayout)
                {
                    ...
                    float edgePadding = GetEdgePadding();
                    float panelPosX = (movementAxis == MovementAxis.Horizontal) ? edgePadding + i * (automaticLayoutSpacing + 1f) * size.x + (size.x / 2f) : 0f;
                    float panelPosY = (movementAxis == MovementAxis.Vertical)   ? edgePadding + i * (automaticLayoutSpacing + 1f) * size.y + (size.y / 2f) : 0f;
            ...
            if (movementType == MovementType.Fixed)
            {
                ...
                if (useAutomaticLayout)
                {
                    ...
                    float edgePadding = GetEdgePadding();
                    float panelsMainSize = movementAxis == MovementAxis.Horizontal
                    ...
                    float contentWidth  = movementAxis == MovementAxis.Horizontal ? panelsMainSize + edgePadding * 2f : size.x;
                    float contentHeight = movementAxis == MovementAxis.Vertical   ? panelsMainSize + edgePadding * 2f : size.y;
        ...
        private float GetEdgePadding()
        {
            return movementAxis == MovementAxis.Horizontal 
                ? Mathf.Max(0f, Viewport.rect.width * 0.5f - size.x * 0.5f) 
                : Mathf.Max(0f, Viewport.rect.height * 0.5f - size.y * 0.5f);
        }
        #endregion
    }
}